...
fn parse_into_card(input: &str) -> Card {
    let (raw_day, raw_numbers) =  split_once_and_trim(input, ':').unwrap();
    let (raw_winning, raw_owned) = split_once_and_trim(raw_numbers, '|').unwrap();

    let day = raw_day.trim_start_matches("Card").trim().parse().unwrap();
...
fn calculate_amount_of_cards(cards: &[Card]) -> u32 {
    const fn card_as_index(card: &Card) -> usize { (card.identifier - 1) as usize }

    let mut card_amounts: Vec<u32> = vec![1; cards.len()];

    for card in cards {
        let card_index = card_as_index(card);
        ...
        for index in next_index..next_index + score {
            let Some(amount) = card_amounts.get_mut(index) else {
                continue
            };
